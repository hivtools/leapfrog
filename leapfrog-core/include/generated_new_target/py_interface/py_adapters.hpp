...
// the `codegen` folder.
// NOLINTBEGIN - ignore entire file as this can't be compiled without python headers
#pragma once
...
struct HvAdapter<Language::Py, real_type, ModelVariant> {
  ...
  static Config::Pars get_pars(
    ...
    return {
      .ex_input = parse_data<real_type, 2>(data, "ex_input", { nda::dim<>(0, SS::pAG, 1), nda::dim<>(0, SS::NS, (SS::pAG)) }),
      .transition_rate = parse_data<real_type, 1>(data, "transition_rate", { nda::dim<>(0, SS::ex, 1) })
    };
  };
  ...
  static Config::State get_initial_state(
    ...
    typename Config::State state;
    fill_initial_state<real_type, typename Config::State::shape_ex_output>(data, "ex_output", state.ex_output);
    fill_initial_state<real_type, typename Config::State::shape_new_output>(data, "new_output", state.new_output);
    return state;
  };
  ...
  static int build_output(
    ...
    const int py_rank_ex_output = 3;
    size_t py_dims_ex_output[py_rank_ex_output] = { SS::pAG, SS::NS, output_years };
    ret["ex_output"] = py_array<real_type>(state.ex_output.data(), py_rank_ex_output, py_dims_ex_output);
    const int py_rank_new_output = 2;
    size_t py_dims_new_output[py_rank_new_output] = { SS::ex, output_years };
    ret["new_output"] = py_array<real_type>(state.new_output.data(), py_rank_new_output, py_dims_new_output);
    return index + output_count;
  };
  ...
  static int build_output_single_year(
    ...
    const int py_rank_ex_output = 2;
    size_t py_dims_ex_output[py_rank_ex_output] = { SS::pAG, SS::NS };
    ret["ex_output"] = py_array<real_type>(state.ex_output.data(), py_rank_ex_output, py_dims_ex_output);
    const int py_rank_new_output = 1;
    size_t py_dims_new_output[py_rank_new_output] = { SS::ex };
    ret["new_output"] = py_array<real_type>(state.new_output.data(), py_rank_new_output, py_dims_new_output);
    return index + output_count;
  };
};
