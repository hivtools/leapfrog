...
// the `codegen` folder.
// NOLINTBEGIN - ignore entire file as this can't be compiled without R headers
#pragma once
...
struct HvAdapter<Language::R, real_type, ModelVariant> {
  ...
  static int build_output(
    ...
    Rcpp::NumericVector r_ex_output(SS::pAG * SS::NS * output_years);
    r_ex_output.attr("dim") = Rcpp::IntegerVector::create(SS::pAG, SS::NS, output_years);
    std::copy_n(state.ex_output.data(), state.ex_output.size(), REAL(r_ex_output));
    names[index + 0] = "ex_output";
    ret[index + 0] = r_ex_output;
    Rcpp::NumericVector r_new_output(SS::ex * output_years);
    r_new_output.attr("dim") = Rcpp::IntegerVector::create(SS::ex, output_years);
    std::copy_n(state.new_output.data(), state.new_output.size(), REAL(r_new_output));
    names[index + 1] = "new_output";
    ret[index + 1] = r_new_output;
    return index + output_count;
  };
  ...
  static int build_output_single_year(
    ...
    Rcpp::NumericVector r_ex_output(SS::pAG * SS::NS);
    r_ex_output.attr("dim") = Rcpp::IntegerVector::create(SS::pAG, SS::NS);
    std::copy_n(state.ex_output.data(), state.ex_output.size(), REAL(r_ex_output));
    names[index + 0] = "ex_output";
    ret[index + 0] = r_ex_output;
    Rcpp::NumericVector r_new_output(SS::ex);
    r_new_output.attr("dim") = Rcpp::IntegerVector::create(SS::ex);
    std::copy_n(state.new_output.data(), state.new_output.size(), REAL(r_new_output));
    names[index + 1] = "new_output";
    ret[index + 1] = r_new_output;
    return index + output_count;
