...
void copy_dim_to_dims_array(hsize_t* array, const std::tuple<Dims...>& tuple) {
  *(array + TIndex) = static_cast<hsize_t>(std::get<sizeof...(Dims) - TIndex - 1>(tuple).extent());
...
nda::array<T, Shape> read_data(
  ...
  dims.resize(static_cast<size_t>(n_dim));
  ...
  for (int i = 0; i < shape.flat_max() + 1; i++) {
    *(ret_data + i) = vec[static_cast<size_t>(i)];
  }
...
template<typename real_type, MV ModelVariant>
  requires(ModelVariant::run_demographic_projection)
struct DpOwnedPars<real_type, ModelVariant> {
...
template<typename real_type, MV ModelVariant>
  requires(ModelVariant::run_hiv_simulation)
struct HaOwnedPars<real_type, ModelVariant> {
  using SS = SSMixed<ModelVariant>;
...
template<typename real_type, MV ModelVariant>
  requires(ModelVariant::run_child_model)
struct HcOwnedPars<real_type, ModelVariant> {
  using SS = SSMixed<ModelVariant>;
...
template<typename real_type, MV ModelVariant>
  requires(ModelVariant::run_goals)
struct HvOwnedPars<real_type, ModelVariant> {
  using SS = SSMixed<ModelVariant>;

  struct Pars {
    using shape_ex_input = nda::shape<nda::dim<0, SS::pAG, 1>, nda::dim<0, SS::NS, SS::pAG>>;
    nda::array<real_type, shape_ex_input> ex_input;
    using shape_transition_rate = nda::shape<nda::dim<0, SS::ex, 1>>;
    nda::array<real_type, shape_transition_rate> transition_rate;
  };
  ...
  static Pars parse_pars(
    ...
    return {
      .ex_input = read_data<real_type, typename Pars::shape_ex_input>(params_file, "ex_input", { nda::dim<>(0, SS::pAG, 1), nda::dim<>(0, SS::NS, SS::pAG) }),
      .transition_rate = read_data<real_type, typename Pars::shape_transition_rate>(params_file, "transition_rate", { nda::dim<>(0, SS::ex, 1) })
    };
  }
};
