...
struct HvAdapter<Language::Cpp, real_type, ModelVariant> {
  ...
  static Config::Pars get_pars(
    ...
    return {
      .ex_input = { owned_pars.hv.ex_input.data(), owned_pars.hv.ex_input.shape() },
      .transition_rate = { owned_pars.hv.transition_rate.data(), owned_pars.hv.transition_rate.shape() }
    };
  }
  ...
  static int build_output(
    ...
    write_data<real_type, typename Config::OutputState::shape_ex_output>(output_file, "ex_output", state.ex_output);
    write_data<real_type, typename Config::OutputState::shape_new_output>(output_file, "new_output", state.new_output);
    return index + output_count;
  }
};
