...
struct HvConfig {
  ...
  struct Pars {
    using shape_ex_input = nda::shape<
      nda::dim<0, SS::pAG, 1>,
      nda::dim<0, SS::NS, (SS::pAG)>
    >;
    nda::array_ref<real_type, shape_ex_input> ex_input;
    using shape_transition_rate = nda::shape<
      nda::dim<0, SS::ex, 1>
    >;
    nda::array_ref<real_type, shape_transition_rate> transition_rate;
  };

  struct Intermediate {
    using shape_ex_intermediate = nda::shape<
      nda::dim<0, SS::pAG, 1>,
      nda::dim<0, SS::NS, (SS::pAG)>
    >;
    nda::array<real_type, shape_ex_intermediate> ex_intermediate;

    Intermediate() {}

    void reset() {
      ex_intermediate.for_each_value([](real_type& x) { x = 0; });
    }
  };

  struct State {
    using shape_ex_output = nda::shape<
      nda::dim<0, SS::pAG, 1>,
      nda::dim<0, SS::NS, (SS::pAG)>
    >;
    nda::array<real_type, shape_ex_output> ex_output;
    using shape_new_output = nda::shape<
      nda::dim<0, SS::ex, 1>
    >;
    nda::array<real_type, shape_new_output> new_output;

    void reset() {
      ex_output.for_each_value([](real_type& x) { x = 0; });
      new_output.for_each_value([](real_type& x) { x = 0; });
    }
  };
  ...
  struct OutputState {
    ...
    using shape_new_output = nda::shape<
      nda::dim<0, SS::ex, 1>,
      nda::dim<0, nda::dynamic, (SS::ex)>
    >;
    nda::array<real_type, shape_new_output> new_output;

    OutputState(int output_years):
      ex_output(shape_ex_output(SS::pAG, SS::NS, output_years)),
      new_output(shape_new_output(SS::ex, output_years))
    {
      ex_output.for_each_value([](real_type& x) { x = 0; });
      new_output.for_each_value([](real_type& x) { x = 0; });
    }
    ...
    void save_state(const size_t i, const State &state) {
      auto chip_ex_output = ex_output(nda::_, nda::_, i);
      nda::for_each_index(chip_ex_output.shape(), [&](auto idx) -> void {
        chip_ex_output[idx] = state.ex_output[idx];
      });
      auto chip_new_output = new_output(nda::_, i);
      nda::for_each_index(chip_new_output.shape(), [&](auto idx) -> void {
        chip_new_output[idx] = state.new_output[idx];
      });
    }
  };
