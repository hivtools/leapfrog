...
concept ChildModelSimulationEnabled = RunDemographicProjection<Config>
    && RunHivSimulation<Config> && RunChildModel<Config>;
...
private:
  ...
  static constexpr auto hc1_to_hc2_cd4_transition =
      SS::hc1_to_hc2_cd4_transition;
  ...
  enum Index {
    ...
    BPLUS_EARLY =
        5,  // PMTCT: Option B+, ART initiated 5-39 weeks before delivery
    BPLUS_LATE = 6,  // PMTCT: Option B+, ART initiated ≤4 weeks before delivery
    NO_ART = 7,  // MTCT: Mother did not receive ART
    MAT_SERO = 8,  // MTCT: Mother seroconverted during pregnancy /
                   // breastfeeding
    BPLUS_BEFORE_DROPOUT = 9,  // Mother on Option B+, ART initiated before
                               // pregnancy but dropped out
    BPLUS_DURING_DROPOUT = 10,  // Mother on Option B+, ART initiated during
                                // pregnancy but dropped out
    ...
    VT_BF_00_05 =
        1,  // Breastfeeding transmission occurring between [6weeks, 2months)
    VT_BF_06_11 =
        2,  // Breastfeeding transmission occurring between [6,11) months
    VT_BF_12_23 =
        3,  // Breastfeeding transmission occurring before [12,23) months
    VT_BF_24_35 =
        4,  // Breastfeeding transmission occurring before [24,36) months
...
public:
  ...
  void run_child_model_simulation() {
    ...
    if (t >= p_hc.hc_art_start) {
      ...
      // Before initiating people on to ART, calculate deaths among CLHIV on ART
      // 6-12 months, the progress them to on ART 6-12 months months and
      // calculate deaths for those on ART >12 months
...
private:
      ...
      for (int a = 1; a < hc2_agestart; ++a) {
        for (int hd = 0; hd < hc1DS; ++hd) {
          for (int cat = 0; cat < hcTT; ++cat) {
            n_hc.hc1_hivpop(hd, cat, a, s) += c_hc.hc1_hivpop(hd, cat, a - 1, s)
                * p_dp.survival_probability(a, s, t);
          }
          for (int dur = 0; dur < hTS; ++dur) {
            n_hc.hc1_artpop(dur, hd, a, s) += c_hc.hc1_artpop(dur, hd, a - 1, s)
                * p_dp.survival_probability(a, s, t);
          }
        }
      }
    ...
    for (int s = 0; s < NS; ++s) {
      for (int hd = 0; hd < hc1DS; ++hd) {
        for (int hd_alt = 0; hd_alt < hc2DS; ++hd_alt) {
          for (int cat = 0; cat < hcTT; ++cat) {
            n_hc.hc2_hivpop(hd_alt, cat, 0, s) +=
                c_hc.hc1_hivpop(hd, cat, (hc2_agestart - 1), s)
                * p_dp.survival_probability(hc2_agestart, s, t)
                * hc1_to_hc2_cd4_transition[hd_alt][hd];
          }
          for (int dur = 0; dur < hTS; ++dur) {
            n_hc.hc2_artpop(dur, hd_alt, 0, s) +=
                c_hc.hc1_artpop(dur, hd, (hc2_agestart - 1), s)
                * p_dp.survival_probability(hc2_agestart, s, t)
                * hc1_to_hc2_cd4_transition[hd_alt][hd];
          }
        }
        ...
        for (int hd = 0; hd < hc2DS; ++hd) {
          for (int cat = 0; cat < hcTT; ++cat) {
            n_hc.hc2_hivpop(hd, cat, a - hc2_agestart, s) +=
                c_hc.hc2_hivpop(hd, cat, a - hc2_agestart - 1, s)
                * p_dp.survival_probability(a, s, t);
          }
          for (int dur = 0; dur < hTS; ++dur) {
            n_hc.hc2_artpop(dur, hd, a - hc2_agestart, s) +=
                c_hc.hc2_artpop(dur, hd, a - hc2_agestart - 1, s)
                * p_dp.survival_probability(a, s, t);
          }
        }
      }
  ...
  void convert_PMTCT_num_to_perc() {
    ...
    // replace all instances of coverage input as numbers with percentage
    // covered
    if (p_hc.PMTCT_input_is_percent(t)) {
      ...
      for (int hp = 0; hp < hPS; ++hp) {
        if (i_hc.sumARV == 0) {
          ...
          i_hc.PMTCT_coverage(hp) =
              p_hc.PMTCT(hp, t) / i_hc.sumARV * i_hc.on_PMTCT / i_hc.need_PMTCT;
          if (hp == BPLUS_BEFORE) {
            // Dropped off ART, started before
            n_hc.mtct_by_source_women(BPLUS_BEFORE_DROPOUT) += p_hc.PMTCT(hp, t)
                * (1 - p_hc.PMTCT_dropout(hp, 0, t));  // number
            // n_hc.mtct_by_source_women(OPTION_A) += p_hc.PMTCT(hp, t) * (1 -
            // p_hc.PMTCT_dropout(hp, 0, t)); //number
            i_hc.PMTCT_before_dropout = i_hc.PMTCT_coverage(hp)
                * (1 - p_hc.PMTCT_dropout(hp, 0, t));  // coverage
          }
          if (hp == BPLUS_EARLY) {
            // Dropped off ART, started during
            n_hc.mtct_by_source_women(BPLUS_DURING_DROPOUT) += p_hc.PMTCT(hp, t)
                * (1 - p_hc.PMTCT_dropout(hp, 0, t));  // number
            i_hc.PMTCT_during_dropout = i_hc.PMTCT_coverage(hp)
                * (1 - p_hc.PMTCT_dropout(hp, 0, t));  // coverage
          }
          i_hc.PMTCT_coverage(hp) *= p_hc.PMTCT_dropout(hp, 0, t);
          n_hc.mtct_by_source_women(hp) +=
              p_hc.PMTCT(hp, t) * p_hc.PMTCT_dropout(hp, 0, t);
        }
  ...
  void convert_PMTCT_pre_bf() {
    ...
    // TODO: Maggie to confirm why Option A/B alt tr aren't used (noted in issue
    // #274)
    for (int hp = 0; hp < hPS; ++hp) {
  ...
  void calc_wlhiv_cd4_proportion() {
    ...
    // Option A and B were only authorized for women with greater than 350 CD4,
    // so if the percentage of women on option A/B > the proportion of women in
    // this cd4 category, we assume that some must have a cd4 less than 350
    ...
    if (p_hc.mat_prev_input(t)) {
      ...
      for (int a = 0; a < hAG_fertility; ++a) {
        i_hc.num_wlhiv_lt200 += n_ha.h_hivpop(4, a, FEMALE)
            + n_ha.h_hivpop(5, a, FEMALE) + n_ha.h_hivpop(6, a, FEMALE);
        i_hc.num_wlhiv_200to350 +=
            n_ha.h_hivpop(3, a, FEMALE) + n_ha.h_hivpop(2, a, FEMALE);
        i_hc.num_wlhiv_gte350 +=
            n_ha.h_hivpop(0, a, FEMALE) + n_ha.h_hivpop(1, a, FEMALE);
      }
      i_hc.num_wlhiv = i_hc.num_wlhiv_200to350 + i_hc.num_wlhiv_gte350
          + i_hc.num_wlhiv_lt200;
  ...
  void adjust_option_A_B_tr() {
    ...
    if (option_A_B_coverage > i_hc.prop_wlhiv_gte350) {
      ...
      i_hc.optA_transmission_rate =
          p_hc.PMTCT_transmission_rate(0, 0, 0) * (1 + i_hc.excessratio);
  ...
  void adjust_option_A_B_bf_tr() {
    ...
    if (i_hc.prop_wlhiv_gte350 > 0) {
      auto option_A_B_coverage =
          i_hc.PMTCT_coverage(0) + i_hc.PMTCT_coverage(1);
      if (option_A_B_coverage > i_hc.prop_wlhiv_gte350) {
        ...
        i_hc.optB_bf_transmission_rate =
            excess_factor_bf * p_hc.PMTCT_transmission_rate(4, OPTION_B, 1);
      } else {
        i_hc.optA_bf_transmission_rate =
            p_hc.PMTCT_transmission_rate(4, OPTION_A, 1);
        i_hc.optB_bf_transmission_rate =
  ...
  void maternal_incidence_in_pregnancy_tr() {
    ...
    if (p_hc.mat_prev_input(t)) {
      for (int a = 0; a < hAG_fertility; ++a) {
        ...
        i_hc.age_weighted_hivneg += asfr_weight
            * p_hc.adult_female_hivnpop(
                a, t
            );  // HIV negative 15-49 women weighted for ASFR
        i_hc.age_weighted_infections += asfr_weight
            * p_hc.adult_female_infections(
                a, t
            );  // newly infected 15-49 women, weighted for ASFR
      }  // end a

      if (i_hc.age_weighted_hivneg > 0.0) {
        i_hc.incidence_rate_wlhiv =
            i_hc.age_weighted_infections / i_hc.age_weighted_hivneg;
        // 0.75 is 9/12, gestational period, index 7 in the vertical
        // transmission object is the index for maternal seroconversion
        i_hc.perinatal_transmission_from_incidence = i_hc.incidence_rate_wlhiv
            * 0.75 * (p_hc.total_births(t) - i_hc.need_PMTCT)
            * p_hc.vertical_transmission_rate(7, 0);
        n_hc.mtct_by_source_women(8) = i_hc.incidence_rate_wlhiv * 0.75
      ...
      if (i_hc.age_weighted_hivneg > 0.0) {
        i_hc.incidence_rate_wlhiv =
            i_hc.age_weighted_infections / i_hc.age_weighted_hivneg;
        // 0.75 is 9/12, gestational period, index 7 in the vertical
        // trasnmission object is the index for maternal seroconversion
        i_hc.perinatal_transmission_from_incidence = i_hc.incidence_rate_wlhiv
            * 0.75 * (n_dp.births - i_hc.need_PMTCT)
            * p_hc.vertical_transmission_rate(7, 0);
      } else {
        i_hc.incidence_rate_wlhiv = 0.0;
        i_hc.perinatal_transmission_from_incidence = 0.0;
  ...
  void perinatal_tr() {
    ...
    for (int hp = 0; hp < hPS; ++hp) {
      if (hp == OPTION_A) {
        n_hc.mtct_by_source_tr(hp, 0) =
            i_hc.PMTCT_coverage(hp) * i_hc.optA_transmission_rate;
      } else if (hp == OPTION_B) {
        n_hc.mtct_by_source_tr(hp, 0) =
            i_hc.PMTCT_coverage(hp) * i_hc.optB_transmission_rate;
      } else {
        n_hc.mtct_by_source_tr(hp, 0) =
            i_hc.PMTCT_coverage(hp) * p_hc.PMTCT_transmission_rate(0, hp, 0);
      }
    ...
    if (i_hc.num_wlhiv > 0) {
      ...
      if ((i_hc.no_PMTCT - i_hc.PMTCT_before_dropout - i_hc.PMTCT_during_dropout
          ...
          > 0)
      {
        n_hc.mtct_by_source_tr(NO_ART, 0) =
            (i_hc.no_PMTCT - i_hc.PMTCT_before_dropout
             - i_hc.PMTCT_during_dropout)
            * untreated_vertical_tr;
      }
      // Dropped off ART
      n_hc.mtct_by_source_tr(BPLUS_BEFORE_DROPOUT, 0) =
          i_hc.PMTCT_before_dropout * untreated_vertical_tr;
      n_hc.mtct_by_source_tr(BPLUS_DURING_DROPOUT, 0) =
    ...
    if (i_hc.need_PMTCT > 0.0) {
      i_hc.perinatal_transmission_rate +=
  ...
  void maternal_incidence_in_bf_tr() {
    ...
    i_hc.bf_incident_hiv_transmission_rate =
        i_hc.bf_at_risk * p_hc.vertical_transmission_rate(7, 1);
  ...
  void run_bf_transmission_rate(int bf_start, int bf_end, int index) {
    ...
    for (int bf = bf_start; bf <= bf_end; bf++) {
      if (bf == 0) {
        // Perinatal transmission accounts for transmission up to 6 weeks, so we
        // only use 1/4 of transmission from the first breastfeeding period
        i_hc.bf_scalar = 0.25;
      } else {
        i_hc.bf_scalar = 1.0;
      ...
      // i_hc.perinatal_transmission_rate_bf_calc is the transmission that has
      // already occurred due to perinatal transmission
      // i_hc.percent_no_treatment is the percentage of women who are still
      // vulnerable to HIV transmission to their babies
      i_hc.percent_no_treatment = 1 - i_hc.perinatal_transmission_rate_bf_calc
      ...
      for (int hp = 0; hp < hPS; hp++) {
        ...
        auto tr = i_hc.PMTCT_coverage(hp)
            * p_hc.PMTCT_transmission_rate(hDS_idx, hp, 1) * 2
            * (1 - p_hc.breastfeeding_duration_art(bf, t)) * i_hc.bf_scalar;
      ...
      if (p_hc.breastfeeding_duration_no_art(bf, t) < 1) {
        ...
        // Never on ART
        auto art_naive = i_hc.percent_no_treatment - i_hc.PMTCT_during_dropout
            - i_hc.PMTCT_before_dropout;
    ...
    for (int s = 0; s < NS; ++s) {
      ...
      for (int a = 0; a < hc2_agestart; ++a) {
        if (p_hc.hc_nosocomial(t) > 0) {
          // 5.0 is used because we want to evenly distribute across the 5 age
          // groups in 0-4
          n_ha.p_infections(a, s) = p_hc.hc_nosocomial(t) / (5.0 * NS);
          // Putting all nosocomial acquired HIV infections in perinatally
          // acquired infection timing and highest CD4 category to match
  ...
  void add_infections() {
    ...
    if (n_ha.hiv_births > 0) {
      ...
      auto perinatal_transmission_births =
          n_ha.hiv_births * i_hc.perinatal_transmission_rate;
      for (int s = 0; s < NS; ++s) {
        for (int hd = 0; hd < hc1DS; ++hd) {
          n_hc.hc1_hivpop(hd, VT_PERINATAL, age_0, s) +=
              perinatal_transmission_births * p_dp.births_sex_prop(s, t)
              * p_hc.hc1_cd4_dist(hd);
        }  // end hc1DS
        auto perinatal_births_by_sex =
            perinatal_transmission_births * p_dp.births_sex_prop(s, t);
        n_ha.p_infections(age_0, s) += perinatal_births_by_sex;
        n_hc.infection_by_type(VT_PERINATAL, age_0, s) +=
            perinatal_births_by_sex;
      }  // end NS
      ...
      // indexing for transmission rates need to have minus one to line up with
      // indexing for hc1/2_hivpop
      // TODO: combine perinatal and breastfeeding transmission rate so the
      ...
      for (int s = 0; s < NS; ++s) {
        ...
        bf_hiv_by_sex += (total_births - n_ha.hiv_births)
            * p_dp.births_sex_prop(s, t)
            * i_hc.bf_incident_hiv_transmission_rate;
        for (int hd = 0; hd < hc1DS; ++hd) {
          n_hc.hc1_hivpop(hd, VT_BF_00_05, age_0, s) +=
              p_hc.hc1_cd4_dist(hd) * bf_hiv_by_sex;
        }  // end hc1DS
      ...
      for (int s = 0; s < NS; ++s) {
        auto bf_hiv_by_sex = n_ha.hiv_births * p_dp.births_sex_prop(s, t)
            * i_hc.bf_transmission_rate((VT_BF_06_11 - 1));
        for (int hd = 0; hd < hc1DS; ++hd) {
          n_hc.hc1_hivpop(hd, VT_BF_06_11, age_0, s) +=
              p_hc.hc1_cd4_dist(hd) * bf_hiv_by_sex;
        }  // end hc1DS
        n_ha.p_infections(age_0, s) += bf_hiv_by_sex;
      ...
      // If the adult model output isn't being used, use the input total
      // population ('bigpop')
      if (p_hc.mat_prev_input(t)) {
        for (int s = 0; s < NS; ++s) {
      ...
      for (int s = 0; s < NS; ++s) {
        ...
        auto uninfected_prop_12_24 =
            (n_dp.p_totpop(age_1, s) - n_ha.p_hivpop(age_1, s))
            / total_pop_12_24;
        scalar_prop_12_24 += uninfected_prop_12_24;
        auto uninfected_prop_24_plus =
            (n_dp.p_totpop(age_2, s) - n_ha.p_hivpop(age_2, s))
            / total_pop_24_plus;
        scalar_prop_24_plus += uninfected_prop_24_plus;

        for (int hd = 0; hd < hc1DS; ++hd) {
          auto bf_hiv_transmission_12_24 = n_ha.hiv_births
              * i_hc.bf_transmission_rate((VT_BF_12_23 - 1))
              * uninfected_prop_12_24;
          auto bf_hiv_transmission_24_plus = n_ha.hiv_births
              * i_hc.bf_transmission_rate((VT_BF_24_35 - 1))
              * uninfected_prop_24_plus;

          // 12-24
          n_hc.hc1_hivpop(hd, VT_BF_12_23, age_1, s) +=
              p_hc.hc1_cd4_dist(hd) * bf_hiv_transmission_12_24;
          n_ha.p_infections(age_1, s) +=
              p_hc.hc1_cd4_dist(hd) * bf_hiv_transmission_12_24;
          n_hc.infection_by_type(VT_BF_12_23, age_1, s) +=
              p_hc.hc1_cd4_dist(hd) * bf_hiv_transmission_12_24;

          // 24 plus
          // TODO: stratify hc1/2_hivpop by BF 12-24 and 24 plus. For now these
          // get put in the same category as 12-24.
          n_hc.hc1_hivpop(hd, VT_BF_12_23, age_2, s) +=
              p_hc.hc1_cd4_dist(hd) * bf_hiv_transmission_24_plus;
          n_ha.p_infections(age_2, s) +=
              p_hc.hc1_cd4_dist(hd) * bf_hiv_transmission_24_plus;
          n_hc.infection_by_type(VT_BF_12_23, age_2, s) +=
              p_hc.hc1_cd4_dist(hd) * bf_hiv_transmission_24_plus;
        }  // end hc1DS
      }  // end NS

      // Sex splitting causes some misalignment, this fixes it but is noted in
      // issue #274
      for (int ms = 0; ms < mtct_source; ms++) {
        n_hc.mtct_by_source_tr(ms, VT_BF_12_23) =
            n_hc.mtct_by_source_tr(ms, VT_BF_12_23) * scalar_prop_12_24;
        n_hc.mtct_by_source_tr(ms, VT_BF_24_35) =
            n_hc.mtct_by_source_tr(ms, VT_BF_24_35) * scalar_prop_24_plus;
      }

      // Fill in infections for the stacked bar output
      for (int ms = 0; ms < mtct_source; ms++) {
        for (int hv = 0; hv < hcTT_expanded; hv++) {
          n_hc.mtct_by_source_hc_infections(ms, hv) =
              n_hc.mtct_by_source_tr(ms, hv) * n_ha.hiv_births;
        }
      }
    }
  };
    ...
    for (int s = 0; s < NS; ++s) {
      for (int a = 0; a < p_hc.hc_art_elig_age(t); ++a) {
        for (int cat = 0; cat < hcTT; ++cat) {
          for (int hd = 0; hd < hc1DS; ++hd) {
            if (a < hc2_agestart) {
              n_hc.hc_art_need_init(hd, cat, a, s) +=
  ...
  void need_for_cotrim() {
    ...
    // Spectrum uses a lagged population and eligibility for children over five
    // (TODO: verify, noted in issue #274)
    for (int s = 0; s < NS; ++s) {
      for (int cat = 0; cat < hcTT; ++cat) {
        for (int a = hc2_agestart; a < hcAG_end; ++a) {
          for (int hd = 0; hd < hc2DS; ++hd) {
            if (a < p_hc.hc_art_elig_age(t)
                || hd >= p_hc.hc_art_elig_cd4(a, t - 1))
            {
  ...
  void cd4_mortality() {
      ...
      for (int a = 0; a < hc2_agestart; ++a) {
        for (int cat = 0; cat < hcTT; ++cat) {
          for (int hd = 0; hd < hc1DS; ++hd) {
            ...
            i_hc.hc_posthivmort(hd, cat, a, s) =
                n_hc.hc1_hivpop(hd, cat, a, s) - hiv_deaths_strat;
          }
        }
      }
    }
    ...
    for (int s = 0; s < NS; ++s) {
      for (int hd = 1; hd < hc1DS; ++hd) {
        for (int a = 0; a < hc2_agestart; ++a) {
          for (int cat = 0; cat < hcTT; ++cat) {
            const auto& coarse_hc1_cd4_prog =
                p_hc.hc1_cd4_prog(hd - 1, hc_age_coarse_cd4[a], s);
            auto cd4_grad = coarse_hc1_cd4_prog
                * (i_hc.hc_posthivmort(hd - 1, cat, a, s)
                   + n_hc.hc1_hivpop(hd - 1, cat, a, s))
                / 2.0;
            i_hc.hc_grad(hd - 1, cat, a, s) -=
                cd4_grad;  // moving to next cd4 category
            i_hc.hc_grad(hd, cat, a, s) +=
                cd4_grad;  // moving into this cd4 category
          }
        }
      }
    }
        ...
        for (int a = hc2_agestart; a < hcAG_end; ++a) {
          for (int cat = 0; cat < hcTT; ++cat) {
            auto cd4_grad = p_hc.hc2_cd4_prog(hd - 1, 0, s)
                * (i_hc.hc_posthivmort(hd - 1, cat, a, s)
                   + n_hc.hc2_hivpop(hd - 1, cat, a - hc2_agestart, s))
                / 2.0;
            i_hc.hc_grad(hd - 1, cat, a, s) -=
                cd4_grad;  // moving to next cd4 category
            i_hc.hc_grad(hd, cat, a, s) +=
      ...
      for (int a = hc2_agestart; a < hcAG_end; ++a) {
        for (int cat = 0; cat < hcTT; ++cat) {
          for (int hd = 0; hd < hc2DS; ++hd) {
            ...
            n_hc.hc2_noart_aids_deaths(hd, cat, a - hc2_agestart, s) +=
                cd4_mort_grad;
          }
        }
      }
        ...
        for (int hd = 0; hd < hc1DS; ++hd) {
          ...
          if (t_art_idx == 0) {
            if (a < hc2_agestart) {
              ...
              bool is_hc2_artpop =
                  n_hc.hc2_artpop(t_art_idx, hd, a - hc2_agestart, s) > 0;
              if (is_hc2_artpop) {
                i_hc.hc_death_rate = p_hc.hc_art_mort_rr(t_art_idx, a, t)
                    * (p_hc.hc2_art_mort(hd, 0, a - hc2_agestart)
                       + p_hc.hc2_art_mort(hd, 1, a - hc2_agestart))
                    / 2.0;
              }
            }
          } else {
            if (a < hc2_agestart) {
              i_hc.hc_death_rate = p_hc.hc_art_mort_rr(t_art_idx, a, t)
                  * p_hc.hc1_art_mort(hd, 2, a);
            } else if (hd < hc2DS) {
              bool is_hc2_artpop =
                  n_hc.hc2_artpop(t_art_idx, hd, a - hc2_agestart, s) > 0;
              if (is_hc2_artpop) {
                i_hc.hc_death_rate = p_hc.hc_art_mort_rr(t_art_idx, a, t)
                    * p_hc.hc2_art_mort(hd, 2, a - hc2_agestart);
              }
            }
          }
          ...
          if (a < hc2_agestart) {
            bool any_hc1_art_deaths =
                i_hc.hc_death_rate * n_hc.hc1_artpop(t_art_idx, hd, a, s) >= 0;
            if (any_hc1_art_deaths) {
              i_hc.hc_art_grad(t_art_idx, hd, a, s) -=
                  i_hc.hc_death_rate * n_hc.hc1_artpop(t_art_idx, hd, a, s);
              n_hc.hc1_artpop(t_art_idx, hd, a, s) +=
                  i_hc.hc_art_grad(t_art_idx, hd, a, s);
              n_hc.hc1_art_aids_deaths(t_art_idx, hd, a, s) -=
                  i_hc.hc_art_grad(t_art_idx, hd, a, s);
            }
          } else if (hd < hc2DS) {
            bool any_hc2_art_deaths = i_hc.hc_death_rate
            ...
            if (any_hc2_art_deaths) {
              ...
              n_hc.hc2_artpop(t_art_idx, hd, a - hc2_agestart, s) +=
                  i_hc.hc_art_grad(t_art_idx, hd, a, s);
              n_hc.hc2_art_aids_deaths(t_art_idx, hd, a - hc2_agestart, s) -=
                  i_hc.hc_art_grad(t_art_idx, hd, a, s);
            }
          }
        }  // end a
          ...
          for (int a = 0; a < hcAG_end; ++a) {
            ...
            if (dur == 0) {
              if (a < hc2_agestart) {
                ...
                bool is_hc2_artpop =
                    n_hc.hc2_artpop(dur, hd, a - hc2_agestart, s) > 0;
                if (is_hc2_artpop) {
                  i_hc.hc_death_rate = p_hc.hc_art_mort_rr(dur, a, t)
                      * (p_hc.hc2_art_mort(hd, 0, a - hc2_agestart)
                         + p_hc.hc2_art_mort(hd, 1, a - hc2_agestart))
            ...
            // ctx reduction on mortality for those on ART
            // NOTE: ART initiation calculations don't include the effect of
            // cotrim (TODO: verify) i_hc.hc_death_rate *=
            // i_hc.ctx_mean(art_flag);
            if (a < hc2_agestart) {
              bool any_hc1_art_deaths =
                  i_hc.hc_death_rate * n_hc.hc1_artpop(dur, hd, a, s) >= 0;
              if (any_hc1_art_deaths) {
                i_hc.hc_art_deaths(hc_age_coarse[a]) +=
                    i_hc.hc_death_rate * n_hc.hc1_artpop(dur, hd, a, s);
              }
            } else if (hd < hc2DS) {
              bool any_hc2_art_deaths = i_hc.hc_death_rate
                      * n_hc.hc2_artpop(dur, hd, a - hc2_agestart, s)
        ...
        for (int s = 0; s < NS; ++s) {
          if (a < hc2_agestart) {
            if (n_hc.hc1_artpop(curr_t_idx, hd, a, s) > 0) {
              n_hc.hc1_artpop(end_t_idx, hd, a, s) +=
                  n_hc.hc1_artpop(curr_t_idx, hd, a, s);
              n_hc.hc1_artpop(curr_t_idx, hd, a, s) -=
                  n_hc.hc1_artpop(curr_t_idx, hd, a, s);
            }
          } else if (hd < hc2DS) {
            n_hc.hc2_artpop(end_t_idx, hd, a - hc2_agestart, s) +=
                n_hc.hc2_artpop(curr_t_idx, hd, a - hc2_agestart, s);
            n_hc.hc2_artpop(curr_t_idx, hd, a - hc2_agestart, s) -=
                n_hc.hc2_artpop(curr_t_idx, hd, a - hc2_agestart, s);
          }
        }  // end NS
  ...
  void calc_total_and_unmet_art_need() {
    ...
    for (int ag = 1; ag < hcAG_coarse; ++ag) {
      ...
      i_hc.total_need(ag) +=
          i_hc.on_art(ag) + i_hc.unmet_need(ag) + i_hc.hc_art_deaths(ag);
    }  // end ag
    i_hc.total_need(0) =
  ...
  void age_specific_art_last_year() {
    ...
    if (p_hc.hc_art_is_age_spec(t - 1)) {
      ...
      for (int s = 0; s < NS; ++s) {
        for (int a = 0; a < hcAG_end; ++a) {
          for (int hd = 0; hd < hc1DS; ++hd) {
            for (int dur = 0; dur < hTS; ++dur) {
              if (a < hc2_agestart) {
                i_hc.total_art_last_year(hc_age_coarse[a]) +=
                    c_hc.hc1_artpop(dur, hd, a, s);
              } else if (hd < hc2DS) {
                i_hc.total_art_last_year(hc_age_coarse[a]) +=
                    c_hc.hc2_artpop(dur, hd, a - hc2_agestart, s);
              }
            }
          }
        }
      ...
      i_hc.total_art_last_year(0) = i_hc.total_art_last_year(1)
          + i_hc.total_art_last_year(2) + i_hc.total_art_last_year(3);
  ...
  void art_last_year() {
    ...
    if (p_hc.hc_art_is_age_spec(t)) {
      // If the present time step is age specific, we need to calculate what
      // last years age spec breakdown would have been
      ...
      if (p_hc.hc_art_isperc(t - 1)) {
        ...
        i_hc.total_art_last_year(0) =
            p_hc.hc_art_val(0, t - 1) * i_hc.total_need(0);
    ...
    for (int s = 0; s < NS; ++s) {
      for (int a = 0; a < hcAG_end; ++a) {
        for (int hd = 0; hd < hc1DS; ++hd) {
          for (int cat = 0; cat < hcTT; ++cat) {
            if (a < hc2_agestart) {
              i_hc.hc_hiv_dist(hd, cat, a, s) +=
                  n_hc.hc1_hivpop(hd, cat, a, s) / i_hc.hc_hiv_total(hd, a, s);
            } else if (hd < hc2DS) {
              i_hc.hc_hiv_dist(hd, cat, a, s) +=
                  n_hc.hc2_hivpop(hd, cat, a - hc2_agestart, s)
                  / i_hc.hc_hiv_total(hd, a, s);
      ...
      for (int a = 0; a < hcAG_end; ++a) {
        for (int hd = 0; hd < hc1DS; ++hd) {
          for (int cat = 0; cat < hcTT; ++cat) {
            if (a < hc2_agestart) {
              auto ltfu_grad =
                  (n_hc.hc1_artpop(2, hd, a, s) + n_hc.hc1_artpop(0, hd, a, s))
                  * p_hc.hc_art_ltfu(t);
              if (i_hc.hc_hiv_total(hd, a, s) > 0) {
                i_hc.art_ltfu_grad(hd, cat, a, s) +=
                    ltfu_grad * i_hc.hc_hiv_dist(hd, cat, a, s);
              } else {
                i_hc.art_ltfu_grad(hd, cat, a, s) += ltfu_grad * 0.25;
              }
            } else if (hd < hc2DS) {
              auto ltfu_grad = (n_hc.hc2_artpop(2, hd, a - hc2_agestart, s)
                                + n_hc.hc2_artpop(0, hd, a - hc2_agestart, s))
                  * p_hc.hc_art_ltfu(t);
              if (i_hc.hc_hiv_total(hd, a, s) > 0) {
                i_hc.art_ltfu_grad(hd, cat, a, s) +=
                    ltfu_grad * i_hc.hc_hiv_dist(hd, cat, a, s);
    ...
    for (int s = 0; s < NS; ++s) {
      for (int a = 0; a < hcAG_end; ++a) {
        for (int hd = 0; hd < hc1DS; ++hd) {
          for (int cat = 0; cat < hcTT; ++cat) {
            if (a < hc2_agestart) {
              // hard coded two as this will only occur among children that are
              // on ART more than a year
              n_hc.hc1_artpop(2, hd, a, s) -= i_hc.art_ltfu_grad(hd, cat, a, s);
            } else if (hd < hc2DS) {
              n_hc.hc2_artpop(2, hd, a - hc2_agestart, s) -=
                  i_hc.art_ltfu_grad(hd, cat, a, s);
            }
          }  // end hcTT
        }  // end hc1DS
    ...
    if (p_hc.hc_art_is_age_spec(t)) {
      for (int s = 0; s < NS; ++s) {
        for (int a = 0; a < hcAG_end; ++a) {
          for (int hd = 0; hd < hc1DS; ++hd) {
            for (int cat = 0; cat < hcTT; ++cat) {
              i_hc.hc_initByAge(hc_age_coarse[a]) +=
                  n_hc.hc_art_need_init(hd, cat, a, s)
        ...
        for (int cat = 0; cat < hcTT; ++cat) {
          for (int a = 0; a < hcAG_end; ++a) {
            for (int hd = 0; hd < hc1DS; ++hd) {
              ...
              auto hc_art_val_sum = p_hc.hc_art_val(0, t)
                  + p_hc.hc_art_val(1, t) + p_hc.hc_art_val(2, t)
                  + p_hc.hc_art_val(3, t);
              auto hc_art_val_sum_last = p_hc.hc_art_val(0, t - 1)
                  + p_hc.hc_art_val(1, t - 1) + p_hc.hc_art_val(2, t - 1)
                  + p_hc.hc_art_val(3, t - 1);
              if (hc_art_val_sum + hc_art_val_sum_last <= 0.0) {
                coarse_hc_art_scalar = 0.0;
              } else {
                coarse_hc_art_scalar =
                    std::min(coarse_hc_adj * p_hc.hc_art_init_dist(a, t), 1.0);
              }

              auto art_initiates =
                  coarse_hc_art_scalar * n_hc.hc_art_need_init(hd, cat, a, s);
      ...
      for (int s = 0; s < NS; ++s) {
        for (int a = 0; a < hcAG_end; ++a) {
          for (int hd = 0; hd < hc1DS; ++hd) {
            for (int cat = 0; cat < hcTT; ++cat) {
              i_hc.hc_initByAge(0) += n_hc.hc_art_need_init(hd, cat, a, s)
                  * p_hc.hc_art_init_dist(a, t);
            }  // end hcTT
          }  // end hc1DS
        }  // end a
      ...
      for (int s = 0; s < NS; ++s) {
        for (int cat = 0; cat < hcTT; ++cat) {
          for (int a = 0; a < hcAG_end; ++a) {
            for (int hd = 0; hd < hc1DS; ++hd) {
              auto hc_art_val_sum =
                  p_hc.hc_art_val(0, t) + p_hc.hc_art_val(0, t - 1);
              if (hc_art_val_sum <= 0) {
                i_hc.hc_art_scalar(0) = 0.0;
              } else {
                i_hc.hc_art_scalar(0) =
                    std::min(i_hc.hc_adj(0) * p_hc.hc_art_init_dist(a, t), 1.0);
              }

              auto art_initiates =
                  i_hc.hc_art_scalar(0) * n_hc.hc_art_need_init(hd, cat, a, s);
    ...
    for (int hd = 0; hd < hDS; ++hd) {
      for (int a = 0; a < hcAG_end; ++a) {
        for (int s = 0; s < NS; ++s) {
          for (int cat = 0; cat < hcTT; ++cat) {
            if (a < hc2_agestart) {
              n_ha.p_hiv_deaths(a, s) +=
                  n_hc.hc1_noart_aids_deaths(hd, cat, a, s);
            } else if (hd < hc2DS) {
              n_ha.p_hiv_deaths(a, s) +=
                  n_hc.hc2_noart_aids_deaths(hd, cat, a - hc2_agestart, s);
            }
          ...
          for (int dur = 0; dur < hTS; ++dur) {
            if (a < hc2_agestart) {
              n_ha.p_hiv_deaths(a, s) +=
                  n_hc.hc1_art_aids_deaths(dur, hd, a, s);
            } else if (hd < hc2DS) {
              n_ha.p_hiv_deaths(a, s) +=
                  n_hc.hc2_art_aids_deaths(dur, hd, a - hc2_agestart, s);
            }
          }  // end dur
        }  // end NS
      }  // end a
