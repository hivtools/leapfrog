...
concept AdultHivModelSimulationEnabled =
...
public:
  ...
  void run_hiv_model_simulation() {
    ...
    i_ha.everARTelig_idx =
        p_ha.idx_hm_elig(t) < hDS ? p_ha.idx_hm_elig(t) : hDS;
    i_ha.anyelig_idx = p_ha.idx_hm_elig(t);

    if (p_ha.incidence_model_choice == SS::INCIDMOD_DIRECTINCID_HTS) {
      // Note: In Spectrum, incidence rate by sex is calculated once per year,
      // using the previous year HIV negative population. Incidence rate by age
      // is calculated per time-step using the **current year** HIV negative
      // population, rathern than the previous year HIV population. Rob
      // Glaubius, 5 August 2022: https://github.com/mrc-ide/leaptfrog/issues/18
...
private:
  void calculate_annual_incidence_rate_by_sex() {
    ...
    for (int s = 0; s < NS; ++s) {
      for (int a = p_ha.pIDX_INCIDPOP;
           a < p_ha.pIDX_INCIDPOP + p_ha.pAG_INCIDPOP;
           ++a)
      {
    ...
    const real_type incrr_wgt_denominator = i_ha.hiv_neg_aggregate(MALE)
        + (p_ha.incidence_rate_ratio_sex(t) * i_ha.hiv_neg_aggregate(FEMALE));
    const real_type total_neg =
        i_ha.hiv_neg_aggregate(MALE) + i_ha.hiv_neg_aggregate(FEMALE);
    i_ha.incidence_rate_sex(MALE) =
        p_ha.input_adult_incidence_rate(t) * total_neg / incrr_wgt_denominator;
    i_ha.incidence_rate_sex(FEMALE) =
        i_ha.incidence_rate_sex(MALE) * p_ha.incidence_rate_ratio_sex(t);
  }
    ...
    for (int s = 0; s < NS; ++s) {
      for (int ha = 0; ha < hAG; ++ha) {
        for (int hm = 0; hm < hDS; ++hm) {
          ...
          auto deaths_hiv = i_ha.cd4mx_scale * p_ha.cd4_mortality(hm, ha, s)
              * n_ha.h_hivpop(hm, ha, s);
          i_ha.h_hiv_deaths_age_sex(ha, s) += opts.dt * deaths_hiv;
          n_ha.h_hiv_deaths_no_art(hm, ha, s) += opts.dt * deaths_hiv;

          auto deaths_excess_nonaids = p_ha.cd4_nonaids_excess_mort(hm, ha, s)
              * n_ha.h_hivpop(hm, ha, s);
          i_ha.h_deaths_excess_nonaids_agesex(ha, s) +=
              opts.dt * deaths_excess_nonaids;
        ...
        for (int hm = 1; hm < hDS; ++hm) {
          const auto hiv_adults_progressing_cd4_stage =
              p_ha.cd4_progression(hm - 1, ha, s)
              * n_ha.h_hivpop(hm - 1, ha, s);
  ...
  void calc_new_infections_incidmod_transmrate(int hiv_step) {
    ...
    real_type Xhivn_s[static_cast<size_t>(NS)];
    real_type Xhivn_incagerr[static_cast<size_t>(NS)];
    ...
    for (int s = 0; s < NS; ++s) {
      ...
      for (int a = SS::pIDX_15to49; a < SS::pIDX_15to49 + SS::pAG_15to49; ++a) {
        ...
        Xhivn_s[s] += Xhivn_sa;
        Xhivn_incagerr[s] +=
            p_ha.incidence_rate_ratio_age(a - p_ha.pIDX_INCIDPOP, s, t)
            * Xhivn_sa;
      }

      for (int ha = SS::hIDX_15to49; ha < SS::hIDX_15to49 + SS::hAG_15to49 + 1;
           ++ha)
      {
        ...
        if (ha == SS::hIDX_15to49) {
          ...
          prop_include = (hivp_ha > 0) ? 1.0
                  - (n_ha.p_hivpop(SS::pIDX_15to49, s) / hivp_ha
          ...
          prop_include = (hivp_ha > 0) ? n_ha.p_hivpop(hAG_start_a, s) / hivp_ha
                  * (1.0 - opts.dt * hiv_step)
                                       : 1.0;
        } else {
          prop_include = 1.0;
    ...
    for (int s = 0; s < NS; s++) {
      Xhivn -= (n_dp.p_totpop(SS::pIDX_15to49, s)
                - n_ha.p_hivpop(SS::pIDX_15to49, s))
          * (1.0 - opts.dt * hiv_step);
    ...
    real_type incrate15to49_hts =
        p_ha.transmission_rate_hts[current_hiv_time_step]
    ...
    n_ha.artcoverage_15to49_hts(hiv_step) =
        Xart + Xhivp_noart > 0.0 ? Xart / (Xart + Xhivp_noart) : 0.0;
    n_ha.prevalence_15to49_hts(hiv_step) = prevcurr;
    n_ha.incidence_15to49_hts(hiv_step) = incrate15to49_hts;

    // incidence by sex
    real_type incrate15to49_s[static_cast<size_t>(NS)];
    incrate15to49_s[MALE] = incrate15to49_hts
        * (Xhivn_s[MALE] + Xhivn_s[FEMALE])
        / (Xhivn_s[MALE] + p_ha.incidence_rate_ratio_sex(t) * Xhivn_s[FEMALE]);
    incrate15to49_s[FEMALE] =
        p_ha.incidence_rate_ratio_sex(t) * incrate15to49_s[MALE];
    ...
    for (int s = 0; s < NS; ++s) {
      for (int a = SS::p_idx_hiv_first_adult; a < pAG; a++) {
        ...
        i_ha.p_infections_ts(a, s) = hivn_a * incrate15to49_s[s]
            * p_ha.incidence_rate_ratio_age(a - SS::p_idx_hiv_first_adult, s, t)
            * Xhivn_s[s] / Xhivn_incagerr[s];
      }
    }
  }
  ...
  void calc_new_infections_agesex() {
    ...
    const auto adult_incid_last_age_group =
        adult_incid_first_age_group + p_ha.pAG_INCIDPOP;
    ...
    for (int s = 0; s < NS; ++s) {
      ...
      for (int a = adult_incid_first_age_group; a < adult_incid_last_age_group;
           ++a)
      {
        i_ha.Xhivn_incagerr +=
      ...
      for (int a = SS::p_idx_hiv_first_adult; a < pAG; ++a) {
        i_ha.p_infections_ts(a, s) = i_ha.hiv_negative_pop(a)
            ...
            * p_ha.incidence_rate_ratio_age(
                a - adult_incid_first_age_group, s, t
            )
            * i_ha.hiv_neg_aggregate(s) / i_ha.Xhivn_incagerr;
      }
    }
  }
  ...
  void add_new_hiv_infections() {
    ...
    for (int s = 0; s < NS; s++) {
      ...
      for (int ha = 0; ha < hAG; ++ha) {
        ...
        for (int hm = 0; hm < hDS; ++hm) {
          i_ha.grad(hm, ha, s) +=
              i_ha.p_infections_ha * p_ha.cd4_initial_distribution(hm, ha, s);
        }
      }
    }
  }
      ...
      for (int ha = 0; ha < hAG; ++ha) {
        for (int hm = i_ha.everARTelig_idx; hm < hDS; ++hm) {
          for (int hu = 0; hu < hTS; ++hu) {
            ...
            const auto deaths_excess_nonaids =
                p_ha.art_nonaids_excess_mort(hu, hm, ha, s)
                * n_ha.h_artpop(hu, hm, ha, s);
            i_ha.h_deaths_excess_nonaids_agesex(ha, s) +=
                opts.dt * deaths_excess_nonaids;
            ...
            i_ha.gradART(hu, hm, ha, s) =
                -(i_ha.deaths_art + deaths_excess_nonaids);
          }

          for (int hu = 0; hu < (hTS - 1); ++hu) {
            const auto art_adults_progressing_treatment_stage =
                n_ha.h_artpop(hu, hm, ha, s) / p_ha.h_art_stage_dur(hu);
            i_ha.gradART(hu, hm, ha, s) -=
                art_adults_progressing_treatment_stage;
            i_ha.gradART(hu + 1, hm, ha, s) +=
                art_adults_progressing_treatment_stage;
          }
          ...
          if (p_ha.dropout_rate(t) > 0) {
            for (int hu = 0; hu < hTS; ++hu) {
              const auto art_adult_dropout =
                  p_ha.dropout_rate(t) * n_ha.h_artpop(hu, hm, ha, s);
  ...
  void run_h_art_initiation(int hiv_step) {
    ...
    for (int s = 0; s < NS; ++s) {
      ...
      for (int ha = hIDX_15PLUS; ha < hAG; ++ha) {
        for (int hm = i_ha.everARTelig_idx; hm < hDS; ++hm) {
          if (hm >= i_ha.anyelig_idx) {
            ...
            real_type tmp_expect_mort = p_ha.cd4_mortality(hm, ha, s)
      ...
      if (opts.proj_period_int == PROJPERIOD_MIDYEAR && art_interp_w < 0.5) {
        if (!p_ha.adults_on_art_is_percent(s, t - 2)
            && !p_ha.adults_on_art_is_percent(s, t - 1))
        {
          // case: both values are numbers
          i_ha.artnum_hts =
              ((0.5 - art_interp_w) * p_ha.adults_on_art(s, t - 2))
              + ((art_interp_w + 0.5) * p_ha.adults_on_art(s, t - 1));
        } else if (p_ha.adults_on_art_is_percent(s, t - 2)
                   && p_ha.adults_on_art_is_percent(s, t - 1))
        {
          ...
          i_ha.artcov_hts =
              ...
              + ((art_interp_w + 0.5) * p_ha.adults_on_art(s, t - 1));
          i_ha.artnum_hts =
              i_ha.artcov_hts * (i_ha.Xart_15plus + i_ha.Xartelig_15plus);
        } else if (!p_ha.adults_on_art_is_percent(s, t - 2)
                   && p_ha.adults_on_art_is_percent(s, t - 1))
        {
          ...
          i_ha.curr_coverage =
              i_ha.Xart_15plus / (i_ha.Xart_15plus + i_ha.Xartelig_15plus);
          i_ha.artcov_hts = i_ha.curr_coverage
              + ((p_ha.adults_on_art(s, t - 1) - i_ha.curr_coverage) * opts.dt
                 / (0.5 - opts.dt * hiv_step));
          // back to number
          i_ha.artnum_hts =
              i_ha.artcov_hts * (i_ha.Xart_15plus + i_ha.Xartelig_15plus);
        }
        ...
        if (!p_ha.adults_on_art_is_percent(s, t - 1)
            && !p_ha.adults_on_art_is_percent(s, t))
        {
          // case: both values are numbers
          i_ha.artnum_hts =
              ((1.0 - art_interp_w) * p_ha.adults_on_art(s, t - 1))
              + (art_interp_w * p_ha.adults_on_art(s, t));
        } else if (p_ha.adults_on_art_is_percent(s, t - 1)
                   && p_ha.adults_on_art_is_percent(s, t))
        {
          // case: both values are percentages
          i_ha.artcov_hts =
                   ...
                   && p_ha.adults_on_art_is_percent(s, t))
        {
          ...
          i_ha.curr_coverage =
              i_ha.Xart_15plus / (i_ha.Xart_15plus + i_ha.Xartelig_15plus);
          i_ha.artcov_hts = i_ha.curr_coverage
              + ((p_ha.adults_on_art(s, t) - i_ha.curr_coverage) * opts.dt
                 / (1.0 - art_interp_w + opts.dt));
          // back to number
          i_ha.artnum_hts =
              i_ha.artcov_hts * (i_ha.Xart_15plus + i_ha.Xartelig_15plus);
        }
      ...
      // Use mixture of eligibility and expected mortality for initiation
      // distribution Spectrum ART allocation is 2-step process
      // 1. Allocate by CD4 category (weighted by 'eligible' and 'expected
      // mortality')
      ...
      for (int hm = i_ha.anyelig_idx; hm < hDS; ++hm) {
        auto eligibility_by_stage = (1.0 - p_ha.initiation_mortality_weight)
            * i_ha.artelig_hm(hm) / i_ha.Xartelig_15plus;
        if (i_ha.expect_mort_artelig15plus > 0) {
          ...
          i_ha.artinit_hm(hm) = i_ha.artinit_hts
              * (eligibility_by_stage + expected_mortality_by_stage);
        ...
        for (int hm = i_ha.anyelig_idx; hm < hDS; ++hm) {
          if (i_ha.artelig_hm(hm) > 0.0) {
            i_ha.artinit_hahm = i_ha.artinit_hm(hm)
                * i_ha.artelig_hahm(hm, ha - hIDX_15PLUS) / i_ha.artelig_hm(hm);
            i_ha.artinit_hahm = std::min(
                i_ha.artinit_hahm, i_ha.artelig_hahm(hm, ha - hIDX_15PLUS)
            );
            i_ha.artinit_hahm = std::min(
                i_ha.artinit_hahm,
  ...
  void run_remove_p_hiv_deaths() {
    ...
    for (int s = 0; s < NS; ++s) {
      ...
      // remove hivdeaths proportionally to age-distribution within each age
      // group
      a = p_idx_hiv_first_adult;
      for (int ha = 0; ha < hAG; ++ha) {
        if (i_ha.hivpop_ha(ha) > 0) {
          i_ha.hivqx_ha = i_ha.h_hiv_deaths_age_sex(ha, s) / i_ha.hivpop_ha(ha);
          auto nonaids_excess_qx_ha =
              i_ha.h_deaths_excess_nonaids_agesex(ha, s) / i_ha.hivpop_ha(ha);

          for (int i = 0; i < hAG_span[static_cast<size_t>(ha)]; ++i, ++a) {
            i_ha.hivdeaths_a = n_ha.p_hivpop(a, s) * i_ha.hivqx_ha;
            auto deaths_nonaids_excess_a =
                n_ha.p_hivpop(a, s) * nonaids_excess_qx_ha;
          ...
          a += hAG_span[static_cast<size_t>(ha)];
        }
      }
    }
  }
  ...
  void run_wlhiv_births() {
    ...
    for (int a = 0; a < hAG_fertility; ++a) {
      ...
      auto total_pop = 0.0;
      auto asfr_w = 0.0;
      for (int a_idx = a_idx_in;
           a_idx < (a_idx_in + hAG_span[static_cast<size_t>(a)]);
           ++a_idx)
      {
        total_pop += n_dp.p_totpop(a_idx, FEMALE);
        asfr_w +=
            p_dp.age_specific_fertility_rate(a_idx - p_idx_fertility_first, t)
            / i_ha.asfr_sum;
      }
      // set up a_idx_in for the next loop
      a_idx_in = a_idx_in + hAG_span[static_cast<size_t>(a)];
      asfr_w /= hAG_span[static_cast<size_t>(a)];

      i_ha.prev = i_ha.nHIVcurr / total_pop;

      for (int hd = 0; hd < hDS; ++hd) {
        i_ha.df += p_ha.local_adj_factor * p_ha.fert_mult_by_age(a, t)
        ...
        i_ha.df += p_ha.local_adj_factor * p_ha.fert_mult_by_age(a, t)
            * p_ha.fert_mult_off_art(hd)
            * (n_ha.h_artpop(0, hd, a, FEMALE) + c_ha.h_artpop(0, hd, a, FEMALE)
            )
            / 2;
        for (int ht = 1; ht < hTS; ++ht) {
          i_ha.df += p_ha.local_adj_factor * p_ha.fert_mult_on_art(a)
              * (n_ha.h_artpop(ht, hd, a, FEMALE)
                 + c_ha.h_artpop(ht, hd, a, FEMALE))
              / 2;
        }  // end hTS
      }  // end hDS
      ...
      n_ha.hiv_births_by_mat_age(a) = midyear_fertileHIV
          * p_dp.total_fertility_rate(t) * i_ha.df
          / (i_ha.df * i_ha.prev + 1 - i_ha.prev) * asfr_w;
