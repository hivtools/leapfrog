...
public:
  ...
  void run_hivpop_end_year_migration() {
    ...
    for (int s = 0; s < NS; ++s) {
      for (int a = 0; a < pAG; ++a) {
        n_ha.p_net_migration_hivpop(a, s) =
            n_ha.p_hivpop(a, s) * i_dp.migration_rate(a, s);
    ...
    for (int s = 0; s < NS; ++s) {
      ...
      for (int ha = 0; ha < hAG; ++ha) {
        ...
        if (hivpop_ha_postmig > 0.0) {
          migration_rate =
              migration_num_ha / (hivpop_ha_postmig - migration_num_ha);
        }
  ...
  void run_hc_hivpop_end_year_migration() {
    static_assert(ModelVariant::run_child_model,
                  "run_hc_hivpop_end_year_migration can only be called for "
                  "model variants where run_child_model is true");
    ...
    for (int s = 0; s < NS; ++s) {
      for (int a = 0; a < hcAG_end; ++a) {
        ...
        if (hc_hivpop_postmig > 0.0) {
          migration_rate =
              hc_migration_num / (hc_hivpop_postmig - hc_migration_num);
        }
...
private:
  void run_hiv_ageing_and_mortality() {
    ...
    for (int s = 0; s < NS; ++s) {
      for (int a = 1; a < pAG; ++a) {
        n_ha.p_deaths_background_hivpop(a, s) = c_ha.p_hivpop(a - 1, s)
  ...
  void run_age_15_entrants() {
    static_assert(ModelVariant::run_child_model,
                  "run_age_15_entrants can only be called for model variants "
                  "where run_child_model is true");
      ...
      for (int hm = 0; hm < hDS; ++hm) {
        ...
        for (int hm_adol = 0; hm_adol < hc2DS; ++hm_adol) {
          ...
          for (int htm = 0; htm < hcTT; ++htm) {
            age15_hivpop_hm_adol +=
                c_hc.hc2_hivpop(hm_adol, htm, (hc2AG - 1), s);
          }
          i_hc.age15_hivpop(hm, s) +=
              age15_hivpop_hm_adol * SS::hc2_to_ha_cd4_transition[hm][hm_adol];
        }
        ...
        if (t > p_hc.hc_art_start) {
          for (int hu = 0; hu < hTS; ++hu) {
            ...
            for (int hm_adol = 0; hm_adol < hc2DS; ++hm_adol) {
              i_hc.age15_artpop(hu, hm, s) +=
                  c_hc.hc2_artpop(hu, hm_adol, (hc2AG - 1), s)
                  * SS::hc2_to_ha_cd4_transition[hm][hm_adol];
            }
          }
        }
      }  // hm
  ...
  void run_hiv_and_art_stratified_ageing() {
    ...
    for (int s = 0; s < NS; ++s) {
      ...
      for (int ha = 0; ha < (hAG - 1); ++ha) {
        ...
        if (i_ha.hiv_age_up_prob(ha, s) > 0) {
          i_ha.hiv_age_up_prob(ha, s) =
              c_ha.p_hivpop(a - 1, s) / i_ha.hiv_age_up_prob(ha, s);
        } else {
    ...
    for (int s = 0; s < NS; ++s) {
      for (int ha = 1; ha < hAG; ++ha) {
        for (int hm = 0; hm < hDS; ++hm) {
          n_ha.h_hivpop(hm, ha, s) = (1.0 - i_ha.hiv_age_up_prob(ha, s))
              * c_ha.h_hivpop(hm, ha, s);  // age-out
          n_ha.h_hivpop(hm, ha, s) += i_ha.hiv_age_up_prob(ha - 1, s)
              * c_ha.h_hivpop(hm, ha - 1, s);  // age-in

          if (t > opts.ts_art_start) {
            for (int hu = 0; hu < hTS; ++hu) {
              n_ha.h_artpop(hu, hm, ha, s) = (1.0 - i_ha.hiv_age_up_prob(ha, s))
                  * c_ha.h_artpop(hu, hm, ha, s);  // age-out
              n_ha.h_artpop(hu, hm, ha, s) += i_ha.hiv_age_up_prob(ha - 1, s)
                  * c_ha.h_artpop(hu, hm, ha - 1, s);  // age-in
            }
          }
        }
      }
          ...
          if (t > p_hc.hc_art_start) {
            for (int hu = 0; hu < hTS; ++hu) {
              if (t > opts.ts_art_start) {
                ...
                // If child ART has started, but not yet adult ART has started,
                // put children on ART into the untreated adult HIV population
    ...
    for (int s = 0; s < NS; ++s) {
      ...
      for (int ha = 0; ha < hAG; ++ha) {
        ...
        if (i_ha.p_coarse_ages_hivpop(ha, s) > 0) {
          deaths_migrate_rate =
              deaths_migrate / i_ha.p_coarse_ages_hivpop(ha, s);
        }
