...
void run_initial_year_calculations(
  ...
  for (int af = 0; af < SS::p_fertility_age_groups; ++af) {
    ...
    // and current age a+1 population survived backwards one year = average of
    // current and previous year population. (Does not account for any initial
    // year net migration)
  ...
  // Note: this calculation would **probably** be more accurate if it survived
  // the population 1 year backwards before calculating deaths (discussed with
  // Rob Glaubius 7 June 2025)
  for (int s = 0; s < SS::NS; ++s) {
    // (a) age 0 deaths
    is_dp.p_deaths_background_totpop(0, s) = is_dp.births
        * p_dp.births_sex_prop(s, t0)
        * (1.0 - p_dp.survival_probability(0, s, t0));

    // (b) age 1 to pAG-1 deaths
    for (int a = 1; a < SS::pAG; ++a) {
      is_dp.p_deaths_background_totpop(a, s) = is_dp.p_totpop(a - 1, s)
          * (1.0 - p_dp.survival_probability(a, s, t0));
    }

    // (c) additional deaths from open-ended age group
    is_dp.p_deaths_background_totpop(SS::pAG - 1, s) +=
        is_dp.p_totpop(SS::pAG - 1, s)
        * (1.0 - p_dp.survival_probability(SS::pAG - 1, s, t0));
  }
}
