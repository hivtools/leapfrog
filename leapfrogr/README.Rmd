...
The simulation model is implemented in a header-only C++ library located in [`leapfrog-core/include/`](leapfrog-core/include/). This location allows the C++ code to be imported in other R packages via specifying `LinkingTo: leapfrog` in the `DESCRIPTION` file.
...
The C++ header library lives outside of the package, which brings a couple of to package installation.

`devtools::load_all()` should work as expected, but before you install the package you need to set `LEAPFROG_INCLUDE` env var to point to the `leapfrog-core/include` directory. There is a helper script for this in `leapfrogr` directory, run

```
./scripts/setup_dev_env.R
```

Note that this won't work for CRAN, CRAN needs a tarball which includes all sources, if and when we build for CRAN we'll need to add a release script which copies the sources into place.
