...
This time output is managed by the state saver see [`leapfrogr/inst/include/state_saver.hpp`](https://github.com/hivtools/leapfrog/blob/main/leapfrogr/inst/include/state_saver.hpp)
...
* Add new input data - modify [`leapfrogr/inst/cpp_generation/model_input.csv`](https://github.com/hivtools/leapfrog/blob/main/leapfrogr/inst/cpp_generation/model_input.csv) and run [`scripts/generate`](https://github.com/hivtools/leapfrog/blob/main/scripts/generate)
* Add new model output - modify [`leapfrogr/inst/cpp_generation/model_output.csv`](https://github.com/hivtools/leapfrog/blob/main/leapfrogr/inst/cpp_generation/model_output.csv) and run [`scripts/generate`](https://github.com/hivtools/leapfrog/blob/main/scripts/generate)

### Input data

Input data is generated from csv file in [`leapfrogr/inst/include/model_input.hpp`](https://github.com/hivtools/leapfrog/blob/main/leapfrogr/inst/include/model_input.hpp), the columns in this file are
...
Note that the dims can be any of the state space sizes or `proj_years` for the number of years in the projection. See the [README](https://hivtools.github.io/leapfrog/#state-space) for valid state space sizes.

After making changes to the [`model_input.csv`](https://github.com/hivtools/leapfrog/blob/main/leapfrogr/inst/cpp_generation/model_input.csv), run the generate script [`scripts/generate`](https://github.com/hivtools/leapfrog/blob/main/scripts/generate). This update file [`leapfrogr/inst/include/model_input.hpp`](https://github.com/hivtools/leapfrog/blob/main/leapfrogr/inst/include/model_input.hpp). Note that this file should never be changed manually as the generate script will completely rewrite it.

After regenerating the code, rebuild the project and you are ready to use the new input data in C++.

### Model outputs

Model outputs are generated in a similar way to inputs. To modify an existing or add new output update the file [`leapfrogr/inst/cpp_generation/model_output.csv`](https://github.com/hivtools/leapfrog/blob/main/leapfrogr/inst/cpp_generation/model_. The columns in this file are:
...
The intermediate data is not handled by code generation as it only requires changes in one place. You can make modifications by adding new data into the struct in [`leapfrogr/inst/include/types.hpp`](https://github.com/hivtools/leapfrog/blob/main/leapfrogr/inst/include/types.hpp), making sure to set its default value in the `reset` function on the struct.
...
1. Add any new input or output data in [`leapfrogr/inst/include/model_input.csv`](https://github.com/hivtools/leapfrog/blob/main/inst/cpp_generation/model_input.csv) or [`leapfrogr/inst/include/model_output.csv`](https://github.com/hivtools/leapfrogr/blob/main/inst/cpp_generation/model_output.csv)
1. If you have new input data, add a specialism for the new variant into the state_types template in [`inst/cpp_generation/state_types.hpp.in`](https://github.com/hivtools/leapfrog/blob/main/inst/cpp_generation/state_types.hpp.in)
1. Run the generate script

In the C++ code:

1. Add your new variant in [`leapfrogr/inst/include/model_variants.hpp`](https://github.com/hivtools/leapfrog/blob/main/leapfrogr/inst/include/model_variants.hpp)
1. Add a struct for the new state space in [`leapfrogr/inst/include/state_space.hpp`](https://github.com/hivtools/leapfrog/blob/main/leapfrogr/inst/include/state_space.hpp)
1. If your model has new input data, add parameters for this variant in [`leapfrogr/inst/include/parameters.hpp`](https://github.com/hivtools/leapfrog/blob/main/leapfrogr/inst/include/parameters.hpp). Parameter types will be generated, you need to add those types to the appropriate template specialisation
1. If your model has new output data, add a new specialisation in [`leapfrogr/inst/include/state_saver.hpp`](https://github.com/hivtools/leapfrog/blob/main/leapfrogr/inst/include/state_saver.hpp)

You now need to update the R wrapper to be able to call this new variant:

1. In [`R/run_model.R`](https://github.com/hivtools/leapfrog/blob/main/leapfrogr/R/run_model.R) add a new flag to the `run_model` interface for using your new variant
1. Update the conditionals to build the `model_variant` string based upon your new flag
1. Add mapping from the string to the model variant struct in Rcpp wrapper code in [`src/leapfrog.cpp`](https://github.com/hivtools/leapfrog/blob/leapfrogr/main/src/leapfrog.cpp). Note use a `constexpr` for the state space to ensure it is evaluated at compile time.
