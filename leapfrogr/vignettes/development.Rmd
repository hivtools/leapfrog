...
* A [public function](https://github.com/hivtools/leapfrog/blob/aa72a70ea4705489325185409223ddfc4aad75ef/leapfrogr/inst/include/models/adult_hiv_model_simulation.hpp#L64) that we can call from `project_year`
...
This time output is managed by the config see e.g. [`leapfrogr/inst/include/generated/config.hpp`](https://github.com/hivtools/leapfrog/blob/aa72a70ea4705489325185409223ddfc4aad75ef/leapfrogr/inst/include/models/adult_hiv_model_simulation.hpp#L110)
...
1. `enable_if` - A conditional for when the model variant should be active. This is a compile time conditional. When the condition is true, the input data must be supplied and output data will be returned.
...
1. `intermediate` - use to define any intermediate bits of data used during model fitting. We define them here because then they can be statically allocated instead of allocated every iteration of the time loop. These are automatically set to 0 at the end of every time step. Each piece of intermediate data needs a "num_type" and "dims".
1. `state` - data output from the model. This is the results filled in during a model fit. Each item needs a "num_type" and (optionally) "dims".
...
1. Add a new flag and variant in [`cpp_generation/modelSchemas/ModelVariants.json`](https://github.com/hivtools/leapfrog/blob/aa72a70ea4705489325185409223ddfc4aad75ef/cpp_generation/modelSchemas/ModelVariants.json)
